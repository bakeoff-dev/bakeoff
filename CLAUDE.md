...
- Diff discipline counts the product change only: test files and documentation (`*.md`, `*.mdx`, `*.rst`, anything under `docs/`) are excluded from both the size and the scope term, so writing the test that proves the fix or updating the docs it makes stale is never charged as diff sprawl. `isTestFile` and `isDocFile` both live in `scorer/tamper.ts`.
